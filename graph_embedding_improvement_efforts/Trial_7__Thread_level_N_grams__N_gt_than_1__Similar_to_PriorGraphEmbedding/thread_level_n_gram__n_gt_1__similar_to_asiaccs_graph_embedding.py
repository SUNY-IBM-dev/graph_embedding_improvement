...
def get_No_Graph_Structure_eventdist_nodetype5bit_adhoc_identifier_dist_dict( dataset : list ):
   #PW: for adhoc features , the sequence of node attr is - relativeName, FileName 
   # Len(RelativeName)=20, len(fileName)=10 
      file_node_tensor = torch.tensor([1, 0, 0, 0, 0])
      reg_node_tensor = torch.tensor([0, 1, 0, 0, 0])
      net_node_tensor = torch.tensor([0, 0, 1, 0, 0])
      proc_node_tensor = torch.tensor([0, 0, 0, 1, 0])
    ...
    parser.add_argument('-ss_opt', '--search_space_option', 
                        choices= [
                                 ...
                                 # ----------- Mean  ----------------------
                                  ...
                                  #------- For updated timestamp dataset----#
                                  'RandomForest_best_case1_nograph',
                                  'RandomForest_best_case2_nograph',
                                  'RandomForest_best_case1_nograph_adhoc'
                                 ...
                                 # "RandomForest_best_hyperparameter_case1"---- use for case1 test
                                  ...
                                  default = ["RandomForest_best_case1_nograph_adhoc"])
...
#PW: Why 10 Kfold? just common values
 # flatten vs no graph ?? is that only ML tuning differece??
    ...
    saved_models_dirpath = "/home/pwakodi1/tabby/Graph_embedding_aka_signal_amplification_files/SILKETW_NEWDATASETs_Results/Adhoc_experiments"   #PW:to change
    ...
    if search_on_train__or__final_test in {"search_on_train", "search_on_all"}:
       experiment_results_df_fpath = f"/home/pwakodi1/tabby/Graph_embedding_aka_signal_amplification_files/SILKETW_NEWDATASETs_Results/Adhoc_experiments/{model_cls_name}__{dataset_choice}__{search_space_option}__{K}_FoldCV__{search_on_train__or__final_test}__{signal_amplification_option}__{readout_option}__{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.csv"
    ...
    if search_on_train__or__final_test == "final_test":
       final_test_results_df_fpath = f"/home/pwakodi1/tabby/Graph_embedding_aka_signal_amplification_files/SILKETW_NEWDATASETs_Results/Adhoc_experiments/{model_cls_name}__{dataset_choice}__{search_space_option}__{search_on_train__or__final_test}__{signal_amplification_option}__{readout_option}__{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.csv"
    ...
    projection_datapath_Benign_Train_dict = {
      ...
      "Dataset-Case-1": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Benign_case1/train",
      # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      "Dataset-Case-2": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Benign_case2/train"
    }
    projection_datapath_Malware_Train_dict = {
      ...
      "Dataset-Case-1": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Malware_case1/train",
      # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      # Dataset-2 (B#662, M#628)
      "Dataset-Case-2": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Malware_case2/train"
    }
    projection_datapath_Benign_Test_dict = {
      ...
      "Dataset-Case-1": \
        # "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Silketw_benign_train_test_data_case1/offline_test/Processed_Benign_ONLY_TaskName_edgeattr",
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Benign_case1/test",
      # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      "Dataset-Case-2": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Benign_case2/test"
    }
    projection_datapath_Malware_Test_dict = {
      ...
      "Dataset-Case-1": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Malware_case1/test",
      # ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
      # Dataset-2 (B#167, M#158)
      "Dataset-Case-2": \
         ...
         "/home/pwakodi1/tabby/SILKETW_DATASET_NEW/Malware_case2/test"
    }
   ...
   #PW: 5 and 62 (61 taskname + 1 timestamp) based on new silketw 
    _dim_node = 35 #46   # num node features ; the #feats #PW: 5 for 5 bit and 36 for adhoc
...
#--------- For Updated timestamp dataset-----------#
    def RandomForest_best_case1_nograph():
      manual_space = []
      manual_space.append(
               {
               'n_estimators': 100, 
    ...
    def RandomForest_best_case2_nograph():
      manual_space = []
      manual_space.append(
               {
               'n_estimators': 300, 
    ...
    def RandomForest_best_case1_nograph_adhoc():
      manual_space = []
      manual_space.append(
               {
               'n_estimators': 200, 
...
#------- for updated timestamp dataset-----#
    elif search_space_option == "RandomForest_best_case1_nograph":
       search_space = RandomForest_best_case1_nograph()   
    elif search_space_option == "RandomForest_best_case2_nograph":
       search_space = RandomForest_best_case2_nograph()   
    elif search_space_option == "RandomForest_best_case1_nograph_adhoc":
       search_space = RandomForest_best_case1_nograph_adhoc()   
        ...
        # Also prepare for final-test dataset, to later test the best-fitted models on test-set
         ...
         if "signal_amplified" in signal_amplification_option:
               ...
               if signal_amplification_option in {"signal_amplified__event_1gram_nodetype_5bit_and_Ahoc_Identifier", 
                                                "no_graph_structure__event_1gram_nodetype_5bit_and_Ahoc_Identifier"}:
                     nodetype_names = ["file", "registry", "network", "process", "thread"] # this is the correct-order
                     feature_names = taskname_colnames + nodetype_names + [f"adhoc_pattern_{i}" for i in range(len(final_test_X.columns) - len(taskname_colnames) - len(nodetype_names))]
