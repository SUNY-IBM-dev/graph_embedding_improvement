...
class GNNBasic(torch.nn.Module):
    ...
    def arguments_read(self, *args, **kwargs):
        ...
        #     x, edge_index, edge_attr, batch = args[0].x, args[0].edge_index, args[0].edge_attr, args[0].batch
        # else:
        ...
        if data:
            ...
            #################################################
            # added by JY @ 2023-05-18 for compatibility with "torch-geometric explaniner"            
            if len(args) == 2:
                x, edge_index = args[0], args[1]
                edge_attr = kwargs['edge_attr']
                batch = None # It's ok to keep batch is None, it's handled in gnn.global_mean_pool()
            #################################################            
...
class GCN(GNNBasic):
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        # batch == 16 
        # "x" will hold index of nodes that correpsond to graph A, B, C, D, E,....
        # edge_index   1-->100, 100->1, 0-->1
        # edge_attr      [ ]       []   []

        # batch == 1

        # "x" will hold index of nodes that correpsond to graph A 
        #     node 0 , node 1 , ... node 100

        # edge_index   1-->100, 100->1, 0-->1
        # edge_attr      [ ]       []   []
 

        # for each edge_attr get the last scalar (temporal-info)
        # then sort, then divide by N=5
        # [ ] [1-->100, ] [ 0-->1] [ ] [ 100->1]
        # edge_attr

        # for each N 
        #     dynamic-graph-mask -- for x only True for dynamic 
...
class GIN_no_edgefeat(GNNBasic):
    ...
    def __init__(self, dim_node : int, dim_hidden : list, num_classes : int, dropout_level : float,
                       ...
                       edge_dim = None, # just for compatibility added @ 2023-06-27
...
class GAT(GNNBasic):
    ...
    def forward(self, *args, **kwargs) -> torch.Tensor:
        ...
        #                 based on "/data/d1/jgwak1/STREAMLINED_DATA_GENERATION_MultiGraph_JY/STEP_3_processdata_traintestsplit/data_preprocessing/dp_v2_ONLY_TASKNAME_EDGE_ATTR/data_processor_v2_MultiEdge_5BitNodeAttr.py"
                ...
                #     node_attr = [0,0,0,0,1]
        Thread_Node_Index = 4
        node_mask_tensor = x[:, Thread_Node_Index] == 1
