...
if __name__ == '__main__':
    ...
    parser.add_argument("--search_on_train__or__final_test", 
                         ...
                         choices= ["search_on_train", "final_test", "search_on_all"],  
    ...
    if search_on_train__or__final_test in {"search_on_train", "search_on_all"}:
       experiment_results_df_fpath = f"/data/d1/jgwak1/stratkfold/{model_cls_name}__{dataset_choice}__{search_space_option}__{K}_FoldCV__{search_on_train__or__final_test}__{signal_amplification_option}__{readout_option}__{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.csv"

    if search_on_train__or__final_test == "final_test":
       final_test_results_df_fpath = f"/data/d1/jgwak1/stratkfold/{model_cls_name}__{dataset_choice}__{search_space_option}__{search_on_train__or__final_test}__{signal_amplification_option}__{readout_option}__{datetime.now().strftime('%Y-%m-%d_%H%M%S')}.csv"
