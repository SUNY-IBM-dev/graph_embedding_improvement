...
def first_step(idx, root_path, EventTypes_to_Exclude_set : set, firststep_hash_debugging_mode : bool = False):
    ...
    es=Elasticsearch(['http://ocelot.cs.binghamton.edu:9200'],timeout = 300)
    scroll = '1m'  # Scroll timeout (1 minute)
    query = {
        "query": {
    ...
    search_results = es.search(
        index='silketwlogs_with_all_provider_chrome',
        scroll=scroll,
        size=100000,  # Number of documents to return in each batch
        body=query
    )
    ...
    while len(search_results['hits']['hits']) > 0:
        ...
        for hit in search_results['hits']['hits']:
            logentry_TaskName = hit['_source']['EventName']
            logentry_Opcode = hit['_source']['Opcode']
            provider = hit["_source"]['ProviderGuid']
            if provider == PROCESS_provider:
                get_proc_info(hit, host_name, 
            ...
            # Access individual document fields using hit['_source']
