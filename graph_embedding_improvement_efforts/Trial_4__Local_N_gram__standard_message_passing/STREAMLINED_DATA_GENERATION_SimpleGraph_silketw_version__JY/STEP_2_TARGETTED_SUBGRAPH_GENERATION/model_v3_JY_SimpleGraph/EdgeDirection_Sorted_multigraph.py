...
def set_edge_dir(graph_root, file_data, net_data, reg_data, proc_data): # 
    ...
    for src , tar , key, edge in edgelist: 
        ...
        if proc_key != None: 
            ...
            if proc_key in {'ProcessStart/Start', 
                            'ProcessStop/Stop', 
                            'JobStart/Start', 
                            'JobStop/Stop', 
        ...
        if net_key!= None: 
            ...
            # 11-Datareceived, 15- connectionaccepted, 43- DatareceivedoverUDPprotocol, 13- Disconnectissued
        ...
        # IF REGISTRY-EVENT -------------------------------------------------------------------------------------------------------
        if reg_key != None:  
            #(PW) 35- QueryKey , 38- QueryValueKey, 39- EnumerateKey , 40- EnumerateValueKey, 
            # 41- QueryMultipleValueKey , 45- QuerySecurityKey
            
            if reg_key in {35, 38, 39, 40, 41, 45}: #PW: added 41, 45 based on SilkEtw
