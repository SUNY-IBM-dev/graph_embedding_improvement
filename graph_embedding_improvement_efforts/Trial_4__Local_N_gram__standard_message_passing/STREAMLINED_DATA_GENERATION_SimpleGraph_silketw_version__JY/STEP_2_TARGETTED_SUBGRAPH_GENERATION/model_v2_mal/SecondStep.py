...
def second_step(root_path):
    ...
    node_all_attrs_defaultvals_dict = {'RelativeName':[0]*20, 'FileName':[0]*15, 'daddr':[0]*6,
                                          'ProcessID':0, 'ProcessId':0,
                                         'ThreadID':0, 'ThreadId':0}
    
    edge_all_attrs_defaultvals_dict = {'StatusCode': [0]*4,'Task Name': [0]*71,
                                    'ReadOperationCount':0,'WriteOperationCount':0,
                                    'ReadTransferKiloBytes':0,'WriteTransferKiloBytes':0,
                                    'size':0, 'Irp':0, 'ImageSize':0,'HandleCount':0, 'Opcode':0,
                                     'TimeStamp':0, 'ImageName':0,'PackageRelativeAppId':[0]*7,
                                     'ProcessTokenElevationType':[0]*3,
                                    'ProcessTokenIsElevated': [0]*2, 'SubProcessTag': [0]*175,
                                    'Status':[0]*34,'Disposition':[0]*3,'MandatoryLabel':[0]*5,}
    ...
    for k, v in edge_dict_raw.items(): # k event uid v= {attr: value}
        ...
        for k1,v1 in v.items():
            ...
            if k1 == 'Task Name':
                ...
                edge_dict_encoded[k][k1] = TN2int(v1,v['Opcode']) # edge_dict_encoded is already encoded at line 1081, so we used 'v'
