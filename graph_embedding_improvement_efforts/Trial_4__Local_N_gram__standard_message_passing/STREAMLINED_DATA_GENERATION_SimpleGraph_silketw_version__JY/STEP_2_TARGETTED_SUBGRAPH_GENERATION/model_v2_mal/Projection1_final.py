...
def benign(root_path,idx,mal_PID,ids,edge_data,proc_node,proc_thread_node): 
    i=0
    backup = []
            ...
            if x not in backup:
                ...
                if key1 == "PROCESSSTART":
                    ...
                    if a.target not in ids:
                        ids.add(a.target)
                        G.vs["taint"]=math.inf
                        root_path = os.path.join(root_path,"Benign_Sample_Noise_P3_"+idx+"_"+str(i))
                        ...
                        tainted_subgraph(root_path,G,current_list,next_list,tainted_nodes,edge_data,edge_min_ts,sub_edge_set,proc_node,proc_thread_node)
            ...
            if k != None:
                if (key1 == "PROCESSSTART") and (key3 == mal_PID):
                    ...
                    if a.target not in ids:
                        ids.add(a.target)
                        G.vs["taint"]=math.inf
                        subgraph_path = os.path.join(root_path,"Malware_Sample_P1_"+idx)
...
def malware_noise(root_path,idx,mal_PID,ids,edge_data,proc_node,proc_thread_node): 
    i = 0 
    G= Graph.Read_GraphML(os.path.join(root_path,"graph.GraphML"))
    for a in G.es:
        name_list= a["name"].replace("[","").replace("]","").replace(" ","").replace("\'","").split(',')
        ...
        if (key1 == "PROCESSSTART") and (key3 != mal_PID):
            ...
            if a.target not in ids:
                ids.add(a.target)
                G.vs["taint"]=math.inf
                subgraph_path = os.path.join(root_path,"Benign_Noise_Sample_P3_"+idx+"_"+str(i))
                if not os.path.exists(subgraph_path):
                    os.mkdir(subgraph_path)
                ts = edge_data.get(name_list[0],{}).get("TimeStamp") #Processstart root node timestamp
                #with open('out_mal1.txt', 'a') as f:
...
def earliest_time(G2,edge_data,node,e1):
    ...
    op = edge_data.get(n[0],{}).get("Opcode")
    if G2.vs[node]["taint"] < ts:
        if (tn == "WRITE" or tn == "CREATE" or tn == "CREATENEWFILE" or 
            tn=="READ" or tn == "QUERYINFORMATION" or tn == "SETINFORMATION" 
            or tn == "QUERYSECURITY" or tn == "SETDELETE" or 
            op == 10 or op == 14 or op == 42 or op == 11 or 
            op == 15 or op == 43 or op == 32 or op == 33 or op == 36 
            or (op==42 and tn=="MICROSOFT-WINDOWS-KERNEL-REGISTRY") or 
            op == 35 or op == 38 or tn == "PROCESSSTART" or tn == "JOBSTART" 
            or tn == "CPUBASEPRIORITYCHANGE" or tn == "CPUPRIORITYCHANGE" or 
            tn == "IMAGELOAD" or tn == "IMAGEUNLOAD" or tn == "THREADSTART" or 
            tn == "IOPRIORITYCHANGE" or tn == "PAGEPRIORITYCHANGE" or 
            tn =="PROCESSFREEZE" ):
            min_ts =ts 
            event1 = tn 
