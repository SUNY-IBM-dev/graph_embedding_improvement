...
class DataProcessor:
    ...
    def process_single_graph(self, file_path, class_label, concat_events=True, compute_order=True):
        ...
        edge_names = None
        if concat_events:
            edge_list, edge_names, freq_vector, NOT_NORMALIZED_count_dict = self.extract_edge_list_v3(graph, edge_dict)  # new version (edge = concat.events)
        ...
        if concat_events and compute_order:
            ...
            edge_attr = self.extract_edge_attr_v3(edge_names, freq_vector, events_order, NOT_NORMALIZED_count_dict)
        else:
    ...
    def extract_edge_list_v3(self, graph, edge_dict):
        ...
        '''
        NOT_NORMALIZED_count_dict = {"ReadOperationCount":read_op_count,"WriteOperationCount": write_op_count,
                                     "ReadTransferKiloBytes":read_transfer_kb , "WriteTransferKiloBytes":write_transfer_kb,
                                      "size":size, "HandleCount": handle_count, "ImageSize":image_size}
        ...
        return edge_list, edge_names, freq_vector, NOT_NORMALIZED_count_dict


    def extract_edge_attr_v3(self, edge_names, freq_vector, events_order, NOT_NORMALIZED_count_dict):
        """
        combines task_freqs with events_order
        """
        edge_attr = []
        ...
        for i in range(len(edge_names)): # looping thru each edge
            # for each edge combines frequency_count with order
            _temp_scalar = [NOT_NORMALIZED_count_dict["ReadOperationCount"][i]] + [NOT_NORMALIZED_count_dict["WriteOperationCount"][i]] + \
                            [NOT_NORMALIZED_count_dict["ReadTransferKiloBytes"][i]] + [NOT_NORMALIZED_count_dict["WriteTransferKiloBytes"][i]] + \
                            [NOT_NORMALIZED_count_dict["size"][i]] + [NOT_NORMALIZED_count_dict["HandleCount"][i]] + [NOT_NORMALIZED_count_dict["ImageSize"][i]]
