...
# JY @ 2023-1-7 : Note that for a PROCESSSTART event,
#                 the newly created process's PID is "ProcessID",
#                 but not "ProcessId" == "Parent Process Id".
def get_create_time(log_entry):
    provider = log_entry.get('_source', {}).get('ProviderId')
    task_name = log_entry.get('_source', {}).get('Task Name')

    if provider == '{22FB2CD6-0E7B-422B-A0C7-2FAD1FD0E716}':
        
        if task_name == 'PROCESSSTART':

            gpid = log_entry.get('_source', {}).get('ProcessId')
            gct = log_entry.get('_source', {}).get('CreateTime')

            create_time_dict[gpid] = gct 
...
## Node Network -------------------------------------------------------------     
def get_net_info(log_entry,host,net_uid, net_thread_uid, net_edge_uid,net_node_dict,net_thread_dict,net_edge_dict):   

    destaddr = log_entry.get('_source', {}).get('daddr') # feature
    ...
    net_event1 = log_entry.get('_source', {}).get('Task Name') # feature
    #net_event.append(net_event1)   
    net_op = log_entry.get('_source', {}).get('Opcode') # feature

    net_tid = log_entry.get('_source', {}).get('ThreadId')
    
    net_pid = log_entry.get('_source', {}).get('ProcessId')

    net_PID = log_entry.get('_source', {}).get('ProcessID')

    net_ts1 = log_entry.get('_source', {}).get('TimeStamp')
    
    size = log_entry.get('_source', {}).get('size')             # feature
    ...
    # np = d.get('_source', {}).get('NewPriority')
    # sb = d.get('_source', {}).get('StackBase')
    spt = log_entry.get('_source', {}).get('SubProcessTag')         #feature
    # usb = d.get('_source', {}).get('UserStackBase')
    # sa = d.get('_source', {}).get('StartAddr')
    # jobid = d.get('_source', {}).get('Job ID')
    # contid = d.get('_source', {}).get('Container ID')
    ...
    #Thread_uid
    create_time_dict=get_create_time(log_entry)
...
## Node Process -------------------------------------------------------------
def get_proc_info(log_entry, host, proc_uid, proc_thread_uid, proc_edge_uid, proc_node_dict, proc_thread_dict, proc_edge_dict):
    # if d.get('_id', {}) == 'HGVxpIMBmvl4jI2n8zhX':
        # print("found HGVxpIMBmvl4jI2n8zhX")
    proc_event1 = log_entry.get('_source', {}).get('Task Name')             # feature
    imgs = log_entry.get('_source', {}).get('ImageSize')                    #feature
    hc = log_entry.get('_source', {}).get('HandleCount')                    #feature
    imgname = log_entry.get('_source', {}).get('ImageName')
    tds = log_entry.get('_source', {}).get('TimeDateStamp')
    proc_PID = log_entry.get('_source', {}).get('ProcessID')
    proc_op = log_entry.get('_source', {}).get('Opcode')    
    ct = log_entry.get('_source', {}).get('CreateTime')
    proc_tid = log_entry.get('_source', {}).get('ThreadId')
    ProcessId = log_entry.get('_source', {}).get('ProcessId')
    proc_ts1 = log_entry.get('_source', {}).get('TimeStamp')


    sc = log_entry.get('_source', {}).get('StatusCode')
    ptet = log_entry.get('_source', {}).get('ProcessTokenElevationType') # feature
    pte = log_entry.get('_source', {}).get('ProcessTokenIsElevated')
    ml = log_entry.get('_source',{}).get('MandatoryLabel') #feature
    pfn = log_entry.get('_source',{}).get('PackageFullName') # feature
    pappid = log_entry.get('_source',{}).get('PackageRelativeAppId')
    tet = log_entry.get('_source',{}).get('TokenElevationType') # feature
    readc = log_entry.get('_source',{}).get('ReadOperationCount') # feature
    writec = log_entry.get('_source',{}).get('WriteOperationCount') # feature
    readt = log_entry.get('_source',{}).get('ReadTransferKiloBytes') # feature
    writet = log_entry.get('_source',{}).get('WriteTransferKiloBytes') # feature
    ...
    ##########################################################################################################################################
    # proc_uid append
    if ProcessId in create_time_dict:  
        CreationTime = create_time_dict[ProcessId]    
        proc_hash = str(ProcessId) + str(CreationTime) 
        proc_out = get_uid(proc_hash,host)
        proc_uid.append(proc_out)
    else:
        proc_hash = str(ProcessId) 
        proc_out = get_uid(proc_hash,host)
        proc_uid.append(proc_out)

    ##########################################################################################################################################
    # Thread UID (TID + Creation time)
    if ProcessId in create_time_dict:                            # JY @ 2022-11-01 : Replaced 'str(proc_pid)' with proc_pid
        CreationTime = create_time_dict[ProcessId]    
        thread_hash = str(proc_tid) + str(ProcessId)+ str(CreationTime) 
        out = get_uid(thread_hash,host)
        proc_thread_uid.append(out)
    else:
        thread_hash = str(proc_tid) + str(ProcessId) 
        out = get_uid(thread_hash,host)
        proc_thread_uid.append(out)

    ##########################################################################################################################################
    # Edge_uid
    hashout = str(ProcessId) + str(proc_tid) + str(proc_ts1)
    eproc_out = get_uid(hashout,host)
    proc_edge_uid.append(eproc_out)
    ##########################################################################################################################################

    # JY @ 2023-1-7 : I think this part is really not straightforward.
    proc_node_dict[proc_out]={'ProcessId': ProcessId}
    ...
    proc_thread_dict[out]= {'ThreadID' : proc_tid, 'ProcessID':proc_PID, 
                            'ThreadId':proc_tid, 'ProcessId':ProcessId}
...
def csv_file(csv_root,
             ...
             file_thread_uid, net_thread_uid, proc_thread_uid, reg_thread_uid,
            
             file_uid, net_uid, proc_uid, reg_uid,

             file_edge_uid, net_edge_uid, proc_edge_uid, reg_edge_uid,):
...
def first_step(idx,root_path):
    ...
    proc_thread_uid = []
    proc_edge_uid = []
    all_hits_val = from_elastic(idx)

    # Gethost
    host_name = get_host()

    # Get providers data
    for num, dic in enumerate(all_hits_val):
        ...
        if provider == '{7DD42A49-59-4832-8DFD-43D979153A8832}':
